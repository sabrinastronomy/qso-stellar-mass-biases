...
Configuration(obs_file=direc_fits + fits_name,
              obsivm_file=direc_fits + f'ivm_{fits_name}',
...
PointSource(xy=Uniform(loc=center-xy_shift, scale=2*xy_shift),
            mag=Uniform(loc=19, scale=6))

## these are all priors, all uniform priors
Sersic(xy=Uniform(loc=center, scale=max_shift),
      mag=Uniform(loc=20, scale=10),
      reff=Uniform(loc=1, scale=10),
      reff_b=Uniform(loc=1, scale=10),
      index=Uniform(loc=0, scale=10),
