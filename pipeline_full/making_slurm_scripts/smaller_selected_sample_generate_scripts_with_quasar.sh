...
for quasar, types in quasar_data.items():
    for obj_type, filters in types.items():
        if not isinstance(filters, dict):
            continue  # skip 'category', 'color', etc.
        is_galaxy = obj_type == 'galaxy'
        for filter, data in (filters or {}).items():
            if data:
                mag_value = float(quasar_data[quasar]['quasar'].get(filter, (0,))[0])
                exp_time = 3100
                wavelength = get_wavelength(filter)
                ss_value = 2 if wavelength > 3 else 1
                galaxy_mag_min = round(float(quasar_data[quasar]['galaxy'].get(filter, (0,))[0]) - 2, 2) if 'galaxy' in quasar_data[quasar] and filter in quasar_data[quasar]['galaxy'] else None
                galaxy_mag_max = round(float(quasar_data[quasar]['galaxy'].get(filter, (0,))[0]) + 2, 2) if 'galaxy' in quasar_data[quasar] and filter in quasar_data[quasar]['galaxy'] else None
...
for line in "${lines[@]}"; do
    ...
    script_path="$output_dir/run_all_mags_${quasar_name}_${quasar_filter}.sh"

    cat <<EOL > "$script_path"
#!/bin/bash
#SBATCH --job-name=run_all_mags_${quasar_name}_${quasar_filter}
#SBATCH --time=10:00:00
#SBATCH --ntasks=10
#SBATCH --cpus-per-task=1
#SBATCH --mem=20G
#SBATCH -D /fred/oz183/sberger/paper_2_obs_bias/src/pipeline_full/
#SBATCH -o /fred/oz183/sberger/paper_2_obs_bias/src/pipeline_full/out_err_files/run_all_mags_${quasar_name}_${quasar_filter}.out
#SBATCH -e /fred/oz183/sberger/paper_2_obs_bias/src/pipeline_full/out_err_files/run_all_mags_${quasar_name}_${quasar_filter}.err
...
python /fred/oz183/sberger/paper_2_obs_bias/src/pipeline_full/run_mags_LARGE_SAMPLE_gal_wrapper.py JWST.NIRCAM.${filter} ${ss} ${mag_value} BY_WIDTH_${quasar_name}_${filter}_photo_cropped_selected_indices_sample_${galaxy_mag_min}_${galaxy_mag_max}.npy ${exp_time} 17
EOL
