...
for quasar, types in quasar_data.items():
    for obj_type, filters in types.items():
        if not isinstance(filters, dict):
            continue  # Skip non-dict keys like 'category'
...
#SBATCH -o /fred/oz183/sberger/paper_2_obs_bias/src/pipeline_full/out_err_files/run_all_just_gal_${quasar_name}_${quasar_filter}.out
#SBATCH -e /fred/oz183/sberger/paper_2_obs_bias/src/pipeline_full/out_err_files/run_all_just_gal_${quasar_name}_${quasar_filter}.err
...
python /fred/oz183/sberger/paper_2_obs_bias/src/pipeline_full/run_just_gal_LARGE_SAMPLE_gal_wrapper.py JWST.NIRCAM.${filter} ${ss} BY_WIDTH_${quasar_name}_${filter}_photo_cropped_selected_indices_sample_${galaxy_mag_min}_${galaxy_mag_max}.npy ${exp_time} 17
EOL
